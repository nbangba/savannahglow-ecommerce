                    ...
                    await fs
                        .runTransaction(async (transaction) => {
                            const docRef = fs
                                ...
                                .doc('ordersMetadata')
                            const ordersMetaDataDoc = await transaction.get(
                                docRef
                            )
                            ...
                            const newNumberOfOrders =
                                ordersMetaDataDoc.data().numberOfOrders + 1

                            let newNumberOfInvoices
                            if (info.payment === 'POD') {
                                newNumberOfInvoices =
                                    ordersMetaDataDoc.data().numberOfInvoices +
                                    1
                                transaction.update(docRef, {
                                    numberOfOrders: newNumberOfOrders,
                                })
                                transaction.update(docRef, {
                                    numberOfInvoices: newNumberOfInvoices,
                                })
                                return {
                                    orderID: newNumberOfOrders,
                                    invoiceID: newNumberOfInvoices,
                                }
                            }
                            ...
                            if (info.payment === 'paystack') {
                                newNumberOfReceipts =
                                    ordersMetaDataDoc.data().numberOfReceipts +
                                    1
                                transaction.update(docRef, {
                                    numberOfOrders: newNumberOfOrders,
                                })
                                transaction.update(docRef, {
                                    numberOfReceipts: newNumberOfReceipts,
                                })

                                return {
                                    orderID: newNumberOfOrders,
                                    receiptID: newNumberOfReceipts,
                                }
                            }
                        })
                        .then((result) =>
            ...
            .then(async (doc) => {
                if (doc.exists) {
                    ...
                    const options = {
                        ...
                        data: {
                            authorization_code:
                                cardData.authorization.authorization_code,
                                ...
                                try {
                                    ...
                                    return await fs
                                        .runTransaction(async (transaction) => {
                                            ...
                                            let newNumberOfReceipts
                                            if (
                                                info.payment === 'paystack' ||
                                                info.paystackOptions ==
                                                    'defaultCard'
                                            ) {
                                                newNumberOfReceipts =
                                                    ordersMetaDataDoc.data()
        ...
        try {
            ...
            await fs
                .runTransaction(async (transaction) => {
                    ...
                    if (info.payment === 'POD') {
                        newNumberOfInvoices =
                            ordersMetaDataDoc.data().numberOfInvoices + 1
                        transaction.update(docRef, {
                            numberOfOrders: newNumberOfOrders,
                        })
                        transaction.update(docRef, {
                    ...
                    if (info.payment === 'paystack') {
                        newNumberOfReceipts =
                            ordersMetaDataDoc.data().numberOfReceipts + 1
                        transaction.update(docRef, {
                            numberOfOrders: newNumberOfOrders,
                        })
                        transaction.update(docRef, {
            ...
            return admin
                ...
                .then(() =>
                    admin
                        ...
                        .then(() => {
                            console.log(`user ${element.displayName} deleted`)
                        })
            ...
            await fs.runTransaction(async (transaction) => {
                ...
                if (values.reviewId) {
                    newRating = parseFloat(
                        (
                            (itemDoc.data().rating *
                                itemDoc.data().numberOfRating +
                                values.rating -
                                values.previousRating) /
                            itemDoc.data().numberOfRating
                        ).toFixed(2)
                    )
                } else {
                    ...
                    newRating = parseFloat(
                        (
                            (itemDoc.data().rating + values.rating) /
                            newNumberOfRating
                        ).toFixed(2)
                    )
                }
