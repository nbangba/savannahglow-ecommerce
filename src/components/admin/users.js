...
function ReactTable({ users }) {
            ...
            // Let's make a column for selection
            {
                ...
                Cell: ({ row }) => {
                    if (
                        (role === 'admin 1' &&
                            (!row.values.role ||
                                row.values.role === 'dispatch')) ||
                        (role === 'admin 2' &&
                            (!row.values.role ||
                                row.values.role === 'dispatch' ||
                                row.values.role === 'admin 1'))
                    )
                        return (
                            <div>
                                <IndeterminateCheckbox
    ...
    const filteredRows = selectedFlatRows.filter(
        (row) => !row.values.role || row.values.role != role
    )
...
*/
    ...
    if (selectedRows.length > 0)
        return (
            <div>
                <Menu style={{ justifyContent: 'center', margin: '20px auto' }}>
                    <MenuItem>
                        <Button
                            secondary
                ...
                <>
                    <AssignRole
                        showAssignRoleDialog={showAssignRoleDialog}
                        setShowAssignRoleDialog={setShowAssignRoleDialog}
                ...
                <Menu style={{ justifyContent: 'center', margin: '20px auto' }}>
                    <MenuItem>
                        <Button
                            secondary
                            style={{ width: 'fit-content' }}
                            onClick={() => assignRole('dispatch', selectedRows)}
                        >
                            Make Dispatch
                        </Button>
                    </MenuItem>
...
export default function Users() {
    ...
    if (users) return <ReactTable users={users} />
    else return <div>No user found</div>
}

function AssignRole({
    showAssignRoleDialog,
    setShowAssignRoleDialog,
    selectedRows,
}) {
    const options = [
