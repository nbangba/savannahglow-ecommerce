...
import { OrderCard } from '../order/orders'
...
export type OrderInfoWithReceiptOrInvoice =
    | OrderInfoWithInvoice
    | OrderInfoWithReceipt
    ...
    useEffect(() => {
        ...
        if (orderStatus == 'all' && (role == 'admin 1' || role == 'admin 2'))
            setqueryOptions(
                query(
                    ordersCollection,
                    orderBy('orderCreated', desc ? 'desc' : 'asc'),
                    where('orderCreated', '>=', orderDate),
...
function Delivery({ order, user }: DeliveryProps) {
    ...
    const onDeliveryFailed = (order: OrderInfoProps, selected: any) => {
        ...
        const newReasonDeliveryFailed = order.reasonsDeliveryFailed
            ? [failedReason, ...order.reasonsDeliveryFailed]
            : [failedReason]
        setDoc(
            doc(firestore, 'orders', order.NO_ID_FIELD),
            {
    ...
    return (
        <>
            <Button onClick={() => setshowModal(true)}>Delivery Failed</Button>
                ...
                <div style={{ minHeight: 300 }}>
                    ...
                    >
                        <Button
                            ...
                            primary
                            onClick={() => {
                                onDeliveryFailed(order, selected)
                                setshowModal(false)
                            }}
                        >
                            Submit
...
function Dispatch({ order }: DispatchProps) {
    ...
    const options =
        users &&
        users.map((user) => {
            return { label: user.displayName, uid: user.uid }
        })
    ...
    const onDispatch = (
        order: OrderInfoProps,
        dispatcher: { label: string; uid: string | null }
    ) => {
        const newDispatcherHistory = order.dispatcherHistory
            ? [dispatcher, ...order.dispatcherHistory]
            : [dispatcher]
    ...
    const handleChange = (selected: any) => {
        if (selected.uid) setselected(selected)
        else setselected({ label: selected.label, uid: null })
    }

    return (
        <>
            <Button onClick={() => setshowModal(true)}>Dispatch</Button>
            <ModalComponent showModal={showModal}>
                <CreatableSelect
                    styles={{
                        container: (base) => ({
